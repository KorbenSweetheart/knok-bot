...
func (p *Processor) Fetch(limit int, timeout int) ([]events.Event, error) {
	...
	p.offset = updates[len(updates)-1].ID + 1 // In the next update, we will get updates after the last one we received before.
...
func meta(event events.Event) (Meta, error) {
	res, ok := event.Meta.(Meta) // type assertion, to check if meta is meta.
	if !ok {
